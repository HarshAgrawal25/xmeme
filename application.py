...
def edit():
    if request.method == "POST":
        ...
        if not url :
            ...
            db.execute("UPDATE xmeme SET url = :values WHERE user_id = :ide", ide = id, values = url)
        row = reversed(db.execute("SELECT * FROM xmeme LIMIT 100"))
...
def editu():
    if request.method == "POST":
        ...
        c = request.form.get("caption")
        if not caption:
            return "Please update any atleast one"
        else:
            db.execute("UPDATE xmeme SET caption = :values WHERE user_id = :ide", ide = id, values = c)
        row = reversed(db.execute("SELECT * FROM xmeme LIMIT 100"))
        return render_template("/index.html", row = row)
